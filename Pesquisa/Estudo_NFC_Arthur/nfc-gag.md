...
## 7. O celular como o próprio hardware

Uma alternativa ao PN532/ESP32 é usar um smartphone comum, mas o comportamento muda entre Android e iPhone:

| Papel | Android | iPhone |
|---------
| **Leitor no balcão** | Leitura nativa via `NfcAdapter` (reader mode), sem hardware externo — funciona em praticamente qualquer aparelho com NFC. Bom candidato a estação fixa do balcão. | Leitura via app com Core NFC exige o app aberto (ou automação via Shortcuts); não serve bem como estação sempre ativa. |
| **Crachá do professor (Card Emulation/HCE)** | `HostApduService` permite que o app do professor emule o crachá, dispensando a tag física. Uma estação Android em reader mode lê normalmente. | **Bloqueado no Brasil.** A Apple só liberou acesso de terceiros ao NFC via HCE dentro do Espaço Econômico Europeu (UE + Noruega/Islândia/Liechtenstein), por exigência antitruste. Fora daí — Brasil incluso — o app precisaria pagar taxa à Apple e usar o Secure Element proprietário, inviável para um projeto acadêmico. |

**Fluxo confirmado — Android como estação fixa:**

- **Professor com Android:** o celular dele emula o crachá (HCE) e encosta na estação Android do balcão — funciona ponta a ponta, sem nenhuma tag física.
- **Professor com iPhone:** precisa de um fallback, já que o iPhone não consegue emular o crachá para a estação Android. Duas opções:

  1. Manter o **crachá físico NTAG** para esses professores (a estação continua a mesma, só muda o que é aproximado dela).
  2. O app do professor no iPhone mostra um **QR code** que a estação lê por câmera em vez de NFC.

**Conclusão prática:** um Android sobrando pode virar a estação leitora do balcão sem custo nenhum de hardware — é uma opção real para o PoC, e funciona 100% via celular para professores com Android. Para professores com iPhone, o crachá físico continua sendo a solução mais simples dentro do escopo NFC do projeto.
